			...
			unsigned long flags, void *context)
{
	...
	dev_err(&chan->dev->device, "%s: sg_len=%d, dir=%s, flags=0x%lx\n",
		 __func__, sg_len,
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		if (!dcb) {
			dev_err(&chan->dev->device, "dma_pool_zalloc failed\n");
		}
...
static int mscc_fdma_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_threaded_irq(&pdev->dev, fdma->irq, NULL,
					 mscc_fdma_interrupt, 0,
	...
	fdma->dcb_pool = dma_pool_create(dev_name(fdma->dma.dev), fdma->dma.dev,
...
err_pool_free:
	dma_pool_destroy(fdma->dcb_pool);
	return ret;
}
